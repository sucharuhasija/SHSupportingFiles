...
-(id) init
{
    if((self = [super init]))
    {
        _thumbnailCache = [IGMDiskCache diskCacheForDirectory:@"thumbs"];
        [_thumbnailCache setMaximumConcurrentNetworkRequests:20];
    }
    return self;
}
...
- (void)imageWithRemotePath:(NSString *)path callback:(ImageCallback)callback
{
    //    NSAssert(path != nil, @"The image path string is nil");
    
    if ( !path ) {
        callback([UIImage imageNamed:@"userPlaceHolder"]);
        return;
    }
    
    [_thumbnailCache loadObjectFromPath:path withCallback:^(NSData *data) {
...
- (void)setImageFromRemotePath:(NSString *)path withImage:(EGOImageView *)imageView
{
    [imageView setShowActivity:YES];
    [imageView.activityIndicator startAnimating];
    if ( !path ) {
        imageView.image = [UIImage imageNamed:@"userPlaceHolder"];
